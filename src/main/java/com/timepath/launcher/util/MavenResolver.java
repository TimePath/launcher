...
public class MavenResolver {
    ...
    static {
        repos = new LinkedHashSet<>(); addRepository(REPO_CENTRAL);
    }
    ...
    public static String resolve(String groupId, String artifactId, String version, String classifier) {
        ...
        groupId = '/' + groupId.replace('.', '/') + '/'; for(String repository : getRepositories()) {
            ...
            String baseVersion = baseArtifact + version + '/'; if(version.endsWith("-SNAPSHOT")) {
                Metadata meta;
                ...
                try {
                    ...
                    LOG.log(Level.WARNING, "{0}", e.toString()); continue;
                } Snapshot snap = meta.getVersioning().getSnapshot(); return MessageFormat.format("{0}{1}-{2}-{3}-{4}{5}",
                                                                                                  baseVersion,
                                                                                                  artifactId,
                                                                                                  version.substring(0,
                                                                                                                    version.lastIndexOf(
    ...
    public static Collection<String> getRepositories() {
        LinkedHashSet<String> repos = new LinkedHashSet<>(); repos.add(REPO_LOCAL); // To allow for changes at runtime
        repos.addAll(MavenResolver.repos); return Collections.unmodifiableCollection(repos);
    }
